...
class QuantityEditableTable(UUIDModel):
    """Durable editable quantity table (R5D-QTO-WORKSPACE-05).

    Stores working-table choices so a user can leave and resume editing.
    Distinct from QuantityPreparationConfig (settings-only) and from
    FiveDModelVersion (immutable read-only freeze). Not BOQ / writeback.
    """

    CONTRACT_VERSION_V1 = "qty-editable-table-v1"
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["project", "-updated_at"]),
            models.Index(fields=["project", "created_by", "-updated_at"]),
            models.Index(fields=["ifc_file"]),
        ]
