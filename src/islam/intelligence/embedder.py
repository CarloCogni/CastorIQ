...
class ScheduleEmbedder:
    ...
    def embed_project(self, project_id: str, force: bool = False) -> dict:
        ...
        existing: dict[str, IslamTaskEmbedding] = {
            str(e.task_id): e
            for e in IslamTaskEmbedding.objects.filter(task__project_id=project_id)
        }
        ...
        for i in range(0, len(to_embed), _BATCH_SIZE):
            ...
            for (task, text), vector in zip(batch, vectors):
                ...
                if rec:
                    ...
                    records.append(IslamTaskEmbedding(task=task, vector=vector, embedded_text=text))

            updates = [r for r in records if r.pk in existing]
            creates = [r for r in records if r.pk not in existing]
            try:
                if updates:
                    IslamTaskEmbedding.objects.bulk_update(
                        updates, ["vector", "embedded_text", "updated_at"]
                    )
                if creates:
                    IslamTaskEmbedding.objects.bulk_create(creates, ignore_conflicts=False)
                embedded += len(batch)
            except Exception:
    ...
    def embed_task(self, task: Task) -> IslamTaskEmbedding | None:
        ...
        rec, _ = IslamTaskEmbedding.objects.update_or_create(
            task=task,
