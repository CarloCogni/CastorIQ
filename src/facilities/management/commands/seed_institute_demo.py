...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(doors) < 4:
            self.stdout.write(
                self.style.WARNING(f"Only {len(doors)} doors found — 3D links limited")
            )
    ...
    def _assets(self, project, user, doors, classes) -> list:
        ...
        door_specs = [
            dict(
                tag="DR-ENT-01",
                name="Main entrance door — Erdgeschoss",
                manufacturer="Hörmann",
                model_number="HT30 glazed",
                serial_number="HRM-ENT-01",
                condition=88,
                cls="Pr_30_59_24",
                warr_days=1400,
                comm_days=600,
            ...
            dict(
                tag="DR-FD30-02",
                name="Fire door FD30 — stairwell",
                manufacturer="Teckentrup",
                model_number="62-1 FSA",
                serial_number="TCK-FD30-02",
                condition=72,
                cls="Pr_30_59_24_30",
                warr_days=900,
                comm_days=600,
            ...
            dict(
                tag="DR-MTG-03",
                name="Meeting room door — 2. Obergeschoss",
                manufacturer="Jeld-Wen",
                model_number="Solid core",
                serial_number="JW-MTG-03",
                condition=90,
                cls="Pr_30_59_24",
                warr_days=1100,
                comm_days=400,
            ...
            dict(
                tag="DR-PLANT-04",
                name="Plant room access door — Keller",
                manufacturer="Novoferm",
                model_number="NovoPorta",
                serial_number="NVF-PLANT-04",
                condition=64,
                cls="Pr_30_59_24_30",
                warr_days=200,
                comm_days=1500,
        ...
        ahu = FacilityAsset.objects.create(
            ...
            asset_tag="AHU-01",
            manufacturer="Wolf",
            model_number="KG Top 40",
            serial_number="WLF-40-11820",
            condition_score=79,
        ...
        self.stdout.write(
            f"Assets: {len(assets)} ({sum(1 for a in assets if a.ifc_entity_id)} linked)"
        )
        return assets
    ...
    def _permits(self, project, user, assets) -> list:
        ...
        specs = [
            dict(
                num="PTW-2001",
                title="Fire door inspection — stairwell",
                kind="other",
                status="active",
                issued="Brandschutz Süd",
                vf=now - dt.timedelta(days=1),
                vu=now + dt.timedelta(days=20),
                asset=1,
            ),
            dict(
                num="PTW-2002",
                title="Electrical isolation — entrance door drive",
                kind="electrical",
                status="active",
                issued="ElektroTechnik GmbH",
                vf=now - dt.timedelta(days=2),
                vu=now + dt.timedelta(days=6),
                asset=0,
            ),
            dict(
                num="PTW-2003",
                title="Working at height — roof AHU access",
                kind="working_at_height",
                status="active",
                issued="Höhenzugang AG",
                vf=now - dt.timedelta(days=1),
                vu=now + dt.timedelta(days=12),
                asset=4,
            ),
            dict(
                num="PTW-2004",
                title="Hot work — plant room door frame weld",
                kind="hot_work",
                status="expired",
                issued="MetallBau",
                vf=now - dt.timedelta(days=30),
                vu=now - dt.timedelta(days=5),
                asset=3,
            ),
            dict(
                num="PTW-2005",
                title="Confined space — basement riser",
                kind="confined_space",
                status="draft",
                issued="",
                vf=None,
    ...
    def _work_orders(self, project, user, assets, permits):
        ...
        specs = [
            dict(
                title="6-monthly fire door inspection — stairwell",
                category="inspection",
                priority=2,
            ...
            dict(
                title="Entrance door closer sluggish — adjust",
                category="corrective",
                priority=2,
            ...
            dict(
                title="Quarterly AHU-01 filter & belt service",
                category="preventive",
                priority=3,
            ...
            dict(
                title="Replace worn seal — plant room door",
                category="corrective",
                priority=3,
            ...
            dict(
                title="Meeting room door — soft-close hinge check",
                category="preventive",
                priority=4,
        ...
        for s in specs:
            ...
            wo = WorkOrder.objects.create(
                ...
                title=s["title"],
                description=(f"Demo work order for the Institute Facilities walkthrough. {MARKER}"),
                category=s["category"],
                priority=s["priority"],
                status=s["status"],
                ...
                due_at=s["due"],
                actual_start=now - dt.timedelta(hours=5)
                if s["status"] >= WorkOrderStatus.IN_PROGRESS
                else None,
                actual_end=now - dt.timedelta(days=3)
    ...
    def _requests(self, project, user, assets):
        specs = [
            ...
            dict(
                title="Upper floors too warm in the afternoon",
                sev="medium",
                status="triaged",
                asset=4,
            ),
