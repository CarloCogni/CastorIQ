...
from islam.scheduling.services.trade_resolver import (
    build_override_map, save_override_map, load_override_map,
)
...
missing = [i for i in confirmed_items if i.get("ai_csi") and i.get("task_id") and i["task_id"] not in om]
if missing:
...
override_types = Counter(
    ...
    for i in confirmed_items if i.get("ai_csi") and i.get("task_id")
)
...
tl_ref = 4848
ok_tl = tl_n == tl_ref
print(f"  {'matches ref 4,848' if ok_tl else f'DRIFT: expected {tl_ref}, got {tl_n}'} {PASS if ok_tl else FAIL}")
...
trade_clusters = sorted(
    drc_coded.get("clusters", {}).get("by_trade", []),
    key=lambda c: -c["downstream_tasks"]
)
largest_trade = trade_clusters[0] if trade_clusters else {}
print(f"\n  Delay root-causes        : {rc_total}  (firm={rc_firm}, lower={rc_lower})")
print(f"  Largest trade cluster    : {largest_trade.get('label','?')}  {largest_trade.get('root_cause_count','?')} root-causes  {largest_trade.get('downstream_tasks','?')} downstream")
ok_drc_total = rc_total == 584
ok_drc_firm  = rc_firm  == 502
ok_drc_lower = rc_lower == 82
print(f"  total=584 {PASS if ok_drc_total else f'{FAIL} got {rc_total}'}  "
...
print(f"  largest cluster=Masonry {PASS if ok_cluster else FAIL + ' got ' + got_label}  "
...
print(f"  total=1597 {PASS if ok_an_total else f'{FAIL} got {an_total}'}  "
...
print(f"  outliers=549 {PASS if ok_an_outlier else f'{FAIL} got {an_outlier}'}  "
...
ok_b03 = b03_bq == 31
ok_l04 = l04_bq == 32
print(f"  B03=31 {PASS if ok_b03 else f'{FAIL} got {b03_bq}'}  "
...
coded_trade_counts  = {}
corr_trade_counts   = {}
for s in tl_coded.get("segments", []):  coded_trade_counts[s["k"]] = coded_trade_counts.get(s["k"],0)+1
for s in tl_corr.get("segments", []):   corr_trade_counts[s["k"]]  = corr_trade_counts.get(s["k"],0)+1
all_keys = set(coded_trade_counts) | set(corr_trade_counts)
changed_trades = {k for k in all_keys if coded_trade_counts.get(k,0) != corr_trade_counts.get(k,0)}
if changed_trades:
    ...
    for k in sorted(changed_trades):
        ...
        print(f"    {k}: {coded_trade_counts.get(k,0)} → {corr_trade_counts.get(k,0)}  ({delta:+d})")
else:
...
trade_clusters_c = sorted(
    drc_corr.get("clusters", {}).get("by_trade", []),
    key=lambda c: -c["downstream_tasks"]
)
...
print(f"  Largest trade cluster    : {largest_c.get('label','?')}  downstream={largest_c.get('downstream_tasks','?')}")
...
print(f"  Statistical outliers     : {an_out_c}  coded={an_outlier}  delta={an_out_c-an_outlier:+d}")
...
print(f"  Floor Health L04 BQ      : {l04_bq_c}  coded={l04_bq}  delta={l04_bq_c-l04_bq if l04_bq_c is not None and l04_bq is not None else 'n/a':}")
...
ok_fh2 = fh2_f.get("B03",{}).get("build_quality",{}).get("score") == b03_bq \
...
print()
print("=" * W)
regression_ok = ok_tl and ok_drc_total and ok_drc_firm and ok_drc_lower \
             and ok_an and ok_b03 and ok_l04 and ok_tl2 and ok_drc2 and ok_an2 and ok_fh2
print("OVERALL REGRESSION:", "PASS ✓  — all as-coded values match baseline" if regression_ok
