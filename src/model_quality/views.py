...
def _storey_registry_ctx(project) -> dict:
    ...
    imported_map = {
        ...
        for lv in Level.objects.filter(project=project, ifc_storey_global_id__isnull=False)
...
class ProgressModeView(ProjectAccessMixin, View):
    ...
    def post(self, request, **kwargs: object) -> HttpResponse:
        ...
        obj, _ = ProgressMode.objects.get_or_create(project=project)
        if obj.mode != mode:
            obj.mode = mode
...
class LevelsView(ProjectTabMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs: object) -> dict:
        ...
        ctx["manual_levels"] = Level.objects.filter(
...
class LevelAddView(ProjectAccessMixin, View):
    """HTMX POST — create or import an Level.
    ...
    def post(self, request, **kwargs: object) -> HttpResponse:
        ...
        if gid:
            level, created = Level.objects.get_or_create(
                ...
                defaults={"name": name, "z_elevation": z, "source": Level.Source.IFC},
            )
            if not created:
                level.name = name
                level.z_elevation = z
...
class LevelEditView(ProjectAccessMixin, View):
    ...
    def post(self, request, level_pk, **kwargs: object) -> HttpResponse:
        project = self.get_project()
        level = get_object_or_404(Level, pk=level_pk, project=project)
...
class LevelApplyView(ProjectAccessMixin, View):
    """POST — write all project Level records back to the IFC file."""
    ...
    def post(self, request, **kwargs: object) -> HttpResponse:
        ...
        levels = list(Level.objects.filter(project=project))
        if not levels:
...
class IssuesLevelsHealthView(ProjectAccessMixin, View):
    ...
    def get(self, request, **kwargs: object) -> HttpResponse:
        ...
        levels_url = reverse("model_quality:levels", kwargs={"pk": project.pk})
        return render(
            request,
