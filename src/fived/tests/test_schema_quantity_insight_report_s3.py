...
"""5D-S3 thin schema quantity insight report page tests."""
...
def test_report_route_200_with_title_contract_and_sections(client):
    """Valid version returns 200 with title, contract, rollups, gaps, non-claims."""
    ...
    assert "5D Schema Quantity Insight" in html
    assert "fived-schema-quantity-insight-s2-v1" in html
    assert 'data-testid="s3-non-claims"' in html
    assert "Not BOQ" in html
    assert "Not cost estimate" in html
    assert 'data-testid="s3-classification-rollup"' in html
    assert 'data-testid="s3-package-rollup"' in html
    assert 'data-testid="s3-work-package-rollup"' in html
    assert 'data-testid="s3-gaps-summary"' in html
    assert 'data-testid="s3-unmapped-counts"' in html
    assert 'data-testid="s3-provenance-counts"' in html
    assert 'data-testid="s3-basis-unit-buckets"' in html
    assert 'data-testid="s3-issue-sample"' in html
    assert 'data-testid="s3-user-value"' in html
    assert 'data-testid="s3-external-input-note"' in html
    assert 'data-testid="s3-help-pill"' in html
    assert "Unmapped" in html
    assert "EL-DEMO-WALL" in html or "Demo Wall" in html
