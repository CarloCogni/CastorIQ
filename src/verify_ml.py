...
from islam.scheduling.services.completion_ml import (
    _train_logreg, _predict_proba, _roc_auc, _brier, _stratified_split,
    _csi, _floor_ordinal, _activity_type, _build_feature_names, _task_row,
    run_completion_ml,
)
from islam.scheduling.models import Task, TaskDependency, P6ResourceAssignment
...
# ── Rebuild raw data ──────────────────────────────────────────────────────────
completed = list(Task.objects.filter(project_id=pid, is_non_physical=False, status='complete')
    .exclude(actual_end=None).exclude(end_date=None).exclude(start_date=None))
...
pred_count_map = dict(TaskDependency.objects.filter(successor_id__in=all_pks)
    .values('successor_id').annotate(n=Count('id')).values_list('successor_id','n'))
succ_count_map = dict(TaskDependency.objects.filter(predecessor_id__in=all_pks)
    .values('predecessor_id').annotate(n=Count('id')).values_list('predecessor_id','n'))
ra_cost = {str(row['task_id']): float(row['total'] or 0) for row in
    P6ResourceAssignment.objects.filter(project_id=pid, is_pending=False)
    .values('task_id').annotate(total=Sum('planned_cost'))}
...
x_all = np.array([
    _task_row(t, pred_count_map.get(str(t.pk),0), succ_count_map.get(str(t.pk),0),
              ra_cost.get(str(t.pk),0.0), trade_rate_loo(_csi(t.activity_code or ''), int(lbl)),
              top_csi, feat_names)
    for t, lbl in zip(completed, labels_all)], dtype=float)
...
# ── 2a. WITH total_float ──────────────────────────────────────────────────────
x_tr_raw = x_all[train_idx]; x_te_raw = x_all[test_idx]
mu_a = x_tr_raw.mean(axis=0); sig_a = x_tr_raw.std(axis=0); sig_a[sig_a==0] = 1.0
...
x_all_nf = x_all[:, keep]
x_tr_nf = x_all_nf[train_idx]; x_te_nf = x_all_nf[test_idx]
mu_b = x_tr_nf.mean(axis=0); sig_b = x_tr_nf.std(axis=0); sig_b[sig_b==0] = 1.0
...
if abs(delta) < 0.03:
    ...
    print(f"  Float contribution is MINOR. AUC=0.744 is not materially inflated.")
    print(f"  No change needed to production model.")
else:
    print(f"  VERDICT: |delta| = {abs(delta):.4f} >= 0.03 threshold.")
...
for t, lbl in zip(completed, labels_all):
    ...
    if a[0] >= 5:
        ...
        print(f"  LOO rate:      ({k_total}-{int(lbl)})/({n_total}-1) = {k_loo}/{n_loo} = {rate_loo:.4f}")
...
inc_tasks = list(Task.objects.filter(project_id=pid, is_non_physical=False)
    .exclude(status='complete').exclude(start_date=None).exclude(end_date=None))
...
if target:
    ...
    print(f"  trade = {_CSI_NAMES.get(_csi(target.activity_code or ''), 'Div ' + _csi(target.activity_code or ''))}")
    print()
    ...
    print(f"  Delta:  {prob - prob_nf:+.3f}  {'(float is driving low prob)' if abs(prob-prob_nf)>0.05 else '(float is NOT the main driver)'}")
