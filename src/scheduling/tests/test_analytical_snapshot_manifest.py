...
class TestSnapshotIBSNoWrite:
    def test_no_ibs_snapshot_rows(self):
        ...
        assert (
            AnalyticalSnapshotResult.objects.filter(snapshot__project_id__in=ibs_ids).count() == 0
        )
        assert (
            AnalyticalSnapshotSeriesPoint.objects.filter(snapshot__project_id__in=ibs_ids).count()
            == 0
        )
