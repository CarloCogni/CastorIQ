...
@pytest.mark.django_db
def test_link_resolver_includes_needs_review_bindings():
    """needs_review bindings count as linked for display purposes."""
    project = ProjectFactory()
    task = TaskFactory(project=project)
    TaskEntityBinding.objects.create(
        task=task,
...
@pytest.mark.django_db
def test_gantt_data_reports_binding_only_task_as_linked(client):
    """GanttDataView uses bindings for link_status and entity_global_ids."""
    user = UserFactory()
