...
"""Trusted-only 4D timeline payloads — summary-first with lazy interval detail.
...
class TimelinePayloadService:
    """Build slim timeline summary and trusted-only interval detail."""
    ...
    def build_interval_detail(
        ...
        """Return trusted GlobalId buckets for one snapshot date."""
        ctx = self._prepare()
        if ctx is None:
            return {
