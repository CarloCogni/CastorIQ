...
from .models import Conflict, GitCommit, ModificationProposal, ScanRun


@admin.register(ModificationProposal)
class ModificationProposalAdmin(admin.ModelAdmin):
    list_display = (
        'id_short', 'request_excerpt', 'ifc_file', 'tier', 'operation',
        'colored_status', 'verification_status', 'created_by',
        'affected_count', 'confidence', 'created_at',
    )
    list_filter = ('status', 'tier', 'verification_status', 'operation', 'created_at', 'ifc_file__project')
    ...
    fieldsets = (
        ...
        ('Review Status', {
            'fields': ('status', 'reviewed_by', 'reviewed_at', 'rejection_reason', 'error_message'),
        }),
        ('Guardian Verification', {
            'fields': ('verification_status', 'verification_result', 'verification_source'),
        }),
        ('Execution', {
            'fields': ('git_commit', 'applied_at'),
            'classes': ('collapse',),
        }),
        ('Timestamps', {
...
class GitCommitAdmin(admin.ModelAdmin):
    ...
    list_filter = ('rolled_back', 'ifc_file__project', 'author', 'created_at')
    ...
    fieldsets = (
        ...
        ('Change Summary', {
            'fields': ('entities_modified', 'entities_added', 'entities_removed', 'rolled_back'),
        }),
        ('Diff Data', {
    ...
    def commit_hash_short(self, obj):
        style = 'color:#991b1b;' if obj.rolled_back else ''
        return format_html('<code style="{}">{}</code>', style, obj.commit_hash[:8])
    ...
    #     # TODO: setup github connection
    #     url = f"https://github.com/CarloCogni/castor/commit/{obj.commit_hash}"
    #     return format_html(
    #         '<a class="button" href="{}" target="_blank" '
    #         'style="background:#24292e; color:white; padding:3px 10px; '
...
class ScanRunAdmin(admin.ModelAdmin):
    list_display = (
        '__str__', 'project', 'scan_type', 'colored_status',
        'entities_scanned', 'chunks_compared', 'conflicts_found',
        'llm_model_used', 'created_at',
    )
    list_filter = ('status', 'scan_type', 'project', 'created_at')
    search_fields = ('error_message', 'llm_model_used')
    readonly_fields = ('id', 'created_at', 'updated_at')
    date_hierarchy = 'created_at'
    list_select_related = ('project', 'triggered_by')
    autocomplete_fields = ['project', 'triggered_by']

    fieldsets = (
        ('Scan Info', {
            'fields': ('id', 'project', 'triggered_by', 'scan_type', 'status', 'llm_model_used'),
        }),
        ('Results', {
            'fields': ('entities_scanned', 'chunks_compared', 'conflicts_found'),
        }),
        ('Errors', {
...
class ConflictAdmin(admin.ModelAdmin):
    list_display = (
        'title', 'severity_badge', 'status_badge', 'project',
        'property_name', 'confidence', 'created_at',
    )
    list_filter = ('severity', 'status', 'project', 'created_at')
    search_fields = ('title', 'description', 'property_name', 'ifc_value', 'document_value')
    readonly_fields = ('id', 'content_hash', 'created_at', 'updated_at')
    date_hierarchy = 'created_at'
    list_select_related = ('project', 'ifc_entity', 'document_chunk', 'resolved_by', 'scan_run')
    ...
    fieldsets = (
        ...
        ('Conflicting Values', {
            'fields': (
                'ifc_entity', 'ifc_value',
                'document_chunk', 'document_value',
                'property_name',
            ),
        }),
        ('Classification', {
            'fields': ('confidence', 'content_hash', 'scan_run'),
        }),
        ('Resolution', {
            'fields': ('resolved_by', 'resolved_at', 'resolution_note', 'suggested_fix'),
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',),
