...
// Sprite part-attachers and enemy/pickup spawn factories, extracted from
// main.js to keep the main module lean. Pure factory module — no module-level
// state, no global mutation. The game scene calls createEntities() once with
// its dependencies and destructures the named functions.
//
// Usage:
//     import { createEntities } from "./entities.js";
//     const entities = createEntities({ k, COLORS, GROUND_Y, ENEMY_SPEED_WALK });
//     const { attachBeaverParts, spawnDupGuid /* ... */ } = entities;
//     attachBeaverParts(parent);
//     spawnDupGuid(worldX, wave);
...
export function createEntities({ k, COLORS, GROUND_Y, ENEMY_SPEED_WALK }) {
    ...
    // ──────────────────────────────────────────────────────────────────────
    // Enemy spawn factories
    //
    // Each spawner takes (worldX, wave). The wave argument scales movement
    // speed; spawners that don't care (stale, token) ignore it.
    // ──────────────────────────────────────────────────────────────────────

    // Dup GUID grunt — baseline ground walker
